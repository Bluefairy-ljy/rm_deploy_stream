...
{
VideoEncoderCore::VideoEncoderCore(const ros::NodeHandle& nh, const ros::NodeHandle& pnh) : nh_(nh), pnh_(pnh), it_(nh_)
{
...
cv::Mat VideoEncoderCore::preprocessImage(const cv::Mat& input, cv::Mat* roi_downsample, cv::Mat* static_removed)
{
  ...
  if (motion_trail_frames_ > 0)
  {
    ...
    if (!suppress_trail && motion_mask_history_.size() > 1 && motion_mask_history_.size() == trail_frame_history_.size())
    {
...
void VideoEncoderCore::imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  try
  {
    if (output_fps_ < 60)
    {
      ...
      if (last_encode_stamp_ns_ > 0 && (now_ns - last_encode_stamp_ns_) < interval_ns)
        return;
...
void VideoEncoderCore::pushFrameToGstreamer(const cv::Mat& frame)
{
  ...
  if (gst_buffer_map(buffer, &map, GST_MAP_WRITE))
  {
    ...
    if (ret != GST_FLOW_OK)
      ROS_WARN("push-buffer failed: %d", ret);
  }
  ...
  while (display_running_ && ros::ok())
  {
    cv::Mat raw, roi, sta, fin;
    {
      ...
      if (!display_raw_frame_.empty())
        display_raw_frame_.copyTo(raw);
      if (!display_roi_frame_.empty())
        display_roi_frame_.copyTo(roi);
      if (!display_static_frame_.empty())
        display_static_frame_.copyTo(sta);
      if (!display_frame_.empty())
        display_frame_.copyTo(fin);
    }

    if (!raw.empty())
      cv::imshow("Doorlock Sniper Raw", raw);
    if (!roi.empty())
      cv::imshow("Doorlock Sniper ROI", roi);
    if (!sta.empty())
      cv::imshow("Doorlock Sniper Static", sta);
    if (!fin.empty())
      cv::imshow("Doorlock Sniper", fin);
    ...
    if (debug_dump_enable_ && !fin.empty())
    {
      ...
      if ((display_frame_counter_ % static_cast<uint64_t>(debug_dump_every_n_frames_)) == 0U)
      {
        ...
        if (debug_dump_save_raw_ && !raw.empty())
          cv::imwrite((dump_dir / ("raw_" + id + ".png")).string(), raw);
        if (debug_dump_save_roi_ && !roi.empty())
          cv::imwrite((dump_dir / ("roi_" + id + ".png")).string(), roi);
        if (debug_dump_save_static_ && !sta.empty())
          cv::imwrite((dump_dir / ("static_" + id + ".png")).string(), sta);
        if (debug_dump_save_final_)
          cv::imwrite((dump_dir / ("final_" + id + ".png")).string(), fin);
      }
    }
