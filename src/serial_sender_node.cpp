...
{
SerialSenderNode::SerialSenderNode() : nh_()
{
  // 参数
  auto serial_port = nh_.param<std::string>("rm_deploy_stream/serial_port", "/dev/ttyACM0");
  int baudrate = nh_.param<int>("rm_deploy_stream/baudrate", 921600);
  packet_size_ = nh_.param<int>("rm_deploy_stream/packet_size", 300);
  // 订阅者
  video_packet_sub_ = nh_.subscribe("/video_stream", 1000, &SerialSenderNode::VideoPacketCB, this);
  // 打开串口
...
void SerialSenderNode::VideoPacketCB(const VideoPacketConstPtr& msg)
{
  // 构造 0x0310 协议帧
  uint16_t total_len = 5 + 2 + packet_size_ + 2;
  std::vector<uint8_t> frame(total_len, 0);
  frame[0] = 0xA5;                                   // SOF
  frame[1] = packet_size_ & 0xFF;                    // data_length low
  frame[2] = (packet_size_ >> 8) & 0xFF;             // data_length high
  frame[3] = static_cast<uint8_t>(msg->seq & 0xFF);  // seq
  frame[4] = 0x00;                                   // CRC8 placeholder
  frame[5] = 0x10;                                   // cmd_id low
  frame[6] = 0x03;                                   // cmd_id high
  memcpy(frame.data() + 7, msg->data.data(), packet_size_);
  // 计算 CRC8，即前4字节
  uint8_t crc8 = CRC8_INIT;
  for (int i = 0; i < 4; ++i)
    crc8 = CRC8_TAB[crc8 ^ frame[i]];
  frame[4] = crc8;
  // 计算 CRC16，即整包减去最后2字节
  uint16_t crc16 = CRC16_INIT;
  for (size_t i = 0; i < total_len - 2; ++i)
  {
    crc16 = (crc16 >> 8) ^ CRC16_TAB[(crc16 ^ frame[i]) & 0xFF];
  }
  frame[total_len - 2] = crc16 & 0xFF;
  frame[total_len - 1] = (crc16 >> 8) & 0xFF;
  // 发送
